...
@app.route("/")
def index():
    s = get_api_rpc('example@gmail.com')
    return s.test2()

    #api.thread.sleep(3.0)
    #return 'test: ' + str(api.thread.counter)
...
def api_get(username, function):
    ...
    try:
        ...
        status = 1
    emit('get', {function: response, status: status})
